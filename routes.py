...
def _selectinload_relationship(attr: Any) -> Any:
    return selectinload(cast(Any, attr))
...
def index():
    ...
    try:
        latest_produtos = (
            ...
            .limit(4) # type: ignore
            .options(selectinload(Produto.imagens)) # type: ignore
            .all()
        )
        for produto in latest_produtos:
            ...
            carousel_media.append(
                {
                    "link_url": url_for("main.detalhe_peca", id=produto.id),
                    "url": media_url,
                    "type": media_type,
                    "filename": fname,
...
def buscar():
    ...
    query = query.options(
        ...
        _selectinload_relationship(Produto.similares),
    )
        ...
        for produto in produtos_paginados:
            ...
            for app in produto.aplicacoes:
                ...
                if aplicacao_termo:
                    ...
                    aplic_ok = (aplicacao_normalizada == veiculo_normalizado or aplicacao_normalizada == motor_normalizado)
    ...
    selected_produto = None
    if pagination.items:
        if selected_id:
            selected_produto = next((p for p in pagination.items if p.id == selected_id), None)
        selected_produto = selected_produto or pagination.items[0]

    # --- Lógica para agrupar conversões por marca ---
    grouped_conversions = collections.defaultdict(list)
    if selected_produto and selected_produto.conversoes:
        # Processa o campo de conversões linha por linha para manter a estrutura original
        linhas = [linha.strip() for linha in selected_produto.conversoes.splitlines() if linha.strip()]
        
        marca_atual = None
        for linha in linhas:
            # Heurística para identificar se a linha é uma marca/fornecedor ou um código.
            e_marca_provavel = _is_conversion_brand_line(linha)

            if e_marca_provavel:
                marca_atual = linha
                # Garante que a marca seja adicionada ao dicionário mesmo se não tiver códigos
                if marca_atual not in grouped_conversions:
                    grouped_conversions[marca_atual] = []
            elif marca_atual:
                # Se já temos uma marca, esta linha é um código associado a ela
                grouped_conversions[marca_atual].append(linha)

    related_products = []
    if selected_produto:
        ignored_rows = (
            db.session.query(SugestaoIgnorada.sugestao_id)
            .filter(SugestaoIgnorada.produto_id == selected_produto.id)
            .all()
        )
        ignored_ids = {r[0] for r in ignored_rows} if ignored_rows else set()

        selected_group_norm = _normalize_for_search(selected_produto.grupo or "")
        selected_supplier_norm = _normalize_for_search(selected_produto.fornecedor or "")
        selected_name_norm = _normalize_for_search(selected_produto.nome or "")
        selected_code_norm = _normalize_for_search(selected_produto.codigo or "")
        selected_measures_norm = _normalize_for_search(selected_produto.medidas or "")

        selected_name_tokens = {
            token
            for token in selected_name_norm.split()
            if len(token) >= 3
        }

        selected_apps = list(selected_produto.aplicacoes or [])
        selected_vehicles = {
        ...
        selected_makers = {
            _normalize_for_search(app.montadora or "")
            for app in selected_apps
            if app.montadora
        }
        selected_vehicle_year_ranges = {}
        for app in selected_apps:
            veiculo_norm = _normalize_for_search(app.veiculo or "")
            year_range = _parse_year_range(app.ano)
            if veiculo_norm and year_range != (-1, -1):
                selected_vehicle_year_ranges.setdefault(veiculo_norm, []).append(year_range)

        ranked_related_map = {}

        def _register_related_candidate(candidate, score):
            if not candidate or candidate.id == selected_produto.id:
                return
            if candidate.id in ignored_ids:
                return
            previous = ranked_related_map.get(candidate.id)
        ...
        for candidate in candidate_pool:
            base_score = _score_product_similarity(candidate)
            if base_score > 0:
                _register_related_candidate(candidate, base_score)

        ranked_related = sorted(
            ranked_related_map.values(),
            key=lambda item: (
                -item["score"],
                _normalize_for_search(item["produto"].codigo or ""),
            ),
        )
        related_products = [item["produto"] for item in ranked_related[:4]]

    related_products_apps_sorted = {}
    if related_products:
        termo_normalizado = _normalize_for_search(termo)
        montadora_normalizada = _normalize_for_search(montadora)
        aplicacao_normalizada = _normalize_for_search(aplicacao_termo)
        selected_vehicles_for_related = set()
        selected_motors_for_related = set()
        selected_makers_for_related = set()
        if selected_produto:
            selected_vehicles_for_related = {
                _normalize_for_search(app.veiculo or "")
                for app in selected_produto.aplicacoes
                if app.veiculo
            }
            selected_motors_for_related = {
                _normalize_for_search(app.motor or "")
                for app in selected_produto.aplicacoes
                if app.motor
            }
            selected_makers_for_related = {
                _normalize_for_search(app.montadora or "")
                for app in selected_produto.aplicacoes
                if app.montadora
            }

        def _score_related_app(app):
            score = 0
            montadora_app = _normalize_for_search(app.montadora or "")
            veiculo_app = _normalize_for_search(app.veiculo or "")
            motor_app = _normalize_for_search(app.motor or "")
            conf_mtr_app = _normalize_for_search(app.conf_mtr or "")
            ano_app = _normalize_for_search(app.ano or "")

            if montadora_normalizada and montadora_app == montadora_normalizada:
                score += 90

            if aplicacao_normalizada:
                if veiculo_app == aplicacao_normalizada:
                    score += 120
                elif aplicacao_normalizada in veiculo_app:
                    score += 80

                if motor_app == aplicacao_normalizada:
                    score += 100
                elif aplicacao_normalizada in motor_app:
                    score += 70

                if conf_mtr_app == aplicacao_normalizada:
                    score += 90
                elif aplicacao_normalizada in conf_mtr_app:
                    score += 60

            if termo_normalizado:
                for campo in (veiculo_app, motor_app, conf_mtr_app, ano_app, montadora_app):
                    if campo == termo_normalizado:
                        score += 30
                    elif termo_normalizado in campo:
                        score += 15

            if veiculo_app and veiculo_app in selected_vehicles_for_related:
                score += 90
            if motor_app and motor_app in selected_motors_for_related:
                score += 55
            if montadora_app and montadora_app in selected_makers_for_related:
                score += 45

            # Critério de desempate leve: favorece linhas com mais dados preenchidos.
            score += int(bool(app.veiculo)) + int(bool(app.ano)) + int(bool(app.motor)) + int(bool(app.conf_mtr))
            return score

        for related in related_products:
            apps_ordenadas = sorted(
                related.aplicacoes,
                key=lambda app: (
                    -_score_related_app(app),
                    _normalize_for_search(app.veiculo or ""),
                    _normalize_for_search(app.ano or ""),
                ),
            )
    ...
    return render_template(
        ...
        sort_by=sort_by,
        sort_dir=sort_dir,
        selected_produto=selected_produto,
        grouped_conversions=dict(grouped_conversions), # Passa os dados agrupados para o template
        related_products=related_products,
        related_products_apps_sorted=related_products_apps_sorted,
...
def detalhe_peca(id):
    ...
    aplicacoes_produto = _relation_list(produto.aplicacoes)
    sorted_aplicacoes = sorted(aplicacoes_produto, key=prioridade_aplicacao)
    ...
    if produto.conversoes:
        linhas = [l.strip() for l in produto.conversoes.splitlines() if l.strip()]
        marca_atual = None
        for linha in linhas:
            e_marca_provavel = _is_conversion_brand_line(linha)
            if e_marca_provavel:
                marca_atual = linha
                if marca_atual not in grouped_conversions_detail:
                    grouped_conversions_detail[marca_atual] = []
            elif marca_atual:
                grouped_conversions_detail[marca_atual].append(linha)
            else:
                grouped_conversions_detail.setdefault("", []).append(linha)
    ...
    if any(x in voltar_url for x in ignore_referrer):
        ...
        )  # Remove a página para voltar à primeira página da busca # type: ignore
        voltar_url = url_for("main.buscar", **search_args)  # type: ignore[arg-type]
...
def adicionar_peca():
    if request.method == "POST":
        ...
        try:
            ...
            novo_produto.grupo = request.form.get("grupo", "").strip().upper()
            novo_produto.fornecedor = request.form.get("fornecedor", "").strip().upper()
            novo_produto.conversoes = request.form.get("conversoes", "").strip().upper()
            novo_produto.medidas = medidas_formatadas
            novo_produto.observacoes = request.form.get("observacoes", "").strip().upper()
            ...
            for file in files:
                if file and file.filename and allowed_file(file.filename):
                    ...
                    imagem_filename = secure_filename( # type: ignore
                    ...
                    nova_imagem.filename = imagem_filename
                    nova_imagem.ordem = proxima_ordem
            ...
            for url_field in ("imagem_url", "imagem_url_2", "imagem_url_3"):
                ...
                if imagem_url:
                    ...
                    if downloaded_filename:
                        nova_imagem = ImagemProduto()
                        nova_imagem.filename = downloaded_filename
                        nova_imagem.ordem = proxima_ordem
                        novo_produto.imagens.append(nova_imagem)
                        proxima_ordem += 1
            ...
            for data in aplicacoes_form.values():
                if data.get("veiculo"):
                    ...
                    for field, value in filtered.items():
                        setattr(nova_aplicacao, field, value)
...
def editar_peca(id):
    produto = (
        db.session.query(Produto)
        .options(
            _selectinload_relationship(Produto.aplicacoes), # type: ignore
            _selectinload_relationship(Produto.imagens), # type: ignore
            _selectinload_relationship(Produto.similares), # type: ignore
        )
        .get(id)
    )
    if not produto:
    ...
    if request.method == "POST":
        ...
        try:
            ...
            aplicacoes_existentes_map = {
                str(app.id): app for app in _relation_list(produto.aplicacoes) if app.id
            }
            ...
            for data in aplicacoes_form.values():
                ...
                if app_id and app_id in aplicacoes_existentes_map:
                    ...
                    nova_aplicacao = Aplicacao()
                    nova_aplicacao.produto_id = produto.id
                    for field, value in filtered_new.items():
                        setattr(nova_aplicacao, field, value)
                    db.session.add(nova_aplicacao)
            ...
            proxima_ordem = max([img.ordem for img in _relation_list(produto.imagens)] + [-1]) + 1
            files = request.files.getlist("imagens")
            for file in files:
                if file and file.filename and allowed_file(file.filename):
                    _, ext = os.path.splitext(file.filename or "")
            ...
            if ordem_imagens_str:
                ...
                imagens_existentes_map = {
                    str(img.id): img for img in _relation_list(produto.imagens) if img.id is not None
                }
                for i, img_id in enumerate(ordem_ids):
                    if img_id in imagens_existentes_map:
                        imagens_existentes_map[img_id].ordem = i

                novas_imagens = [img for img in _relation_list(produto.imagens) if img.id is None]
...
def clonar_peca(id):
    ...
    novo_produto = Produto()
    novo_produto.nome = produto_original.nome
    novo_produto.codigo = f"{produto_original.codigo}-{clone_suffix}"
    novo_produto.grupo = produto_original.grupo
    novo_produto.fornecedor = produto_original.fornecedor
    novo_produto.conversoes = produto_original.conversoes
    novo_produto.medidas = produto_original.medidas
    novo_produto.observacoes = produto_original.observacoes

    for imagem_original in _relation_list(produto_original.imagens):
        nova_imagem = ImagemProduto()
        nova_imagem.filename = imagem_original.filename
        nova_imagem.ordem = imagem_original.ordem
        novo_produto.imagens.append(nova_imagem)
...
def excluir_peca(id):
    ...
    try:
        # 1. Remove todas as sugestões ignoradas relacionadas a este produto
        # (tanto como produto_id quanto como sugestao_id)
        db.session.query(SugestaoIgnorada).filter(
            db.or_(
                SugestaoIgnorada.produto_id == id,
                SugestaoIgnorada.sugestao_id == id
            )
        ).delete()

        # 2. Remove relações de similaridade (ambos os lados)
        db.session.query(similares_association).filter(
            db.or_(
                similares_association.c.produto_id == id,
                similares_association.c.similar_id == id
            )
        ).delete()

        # 3. Deleta as imagens do disco
        if produto.imagens:
            filenames_to_check = {img.filename for img in _relation_list(produto.imagens)}
...
def remover_similar(produto_id, similar_id):
    ...
    try:
        ...
        if similar in produto.similares: # type: ignore
            produto.similares.remove(similar)
        if produto in similar.similares: # type: ignore
...
def adicionar_aplicacao():
    ...
    if request.method == "POST":
        ...
        nova = Aplicacao()
        nova.produto_id = produto.id
        nova.veiculo = veiculo
        nova.ano = ano
        nova.motor = motor
        nova.conf_mtr = conf_mtr
        nova.montadora = montadora
        db.session.add(nova)
        db.session.commit()
...
def adicionar_usuario():
    ...
    if User.query.filter_by(username=username).first():
        ...
        new_user = User( # type: ignore
            ...
            is_admin=request.form.get("is_admin") == "on", # type: ignore
        )
...
def configuracoes():
    ...
    if request.method == "POST":
        ...
        if "logo" in request.files:
            ...
            if file and file.filename and allowed_file(file.filename):
                logo_filename = "logo" + os.path.splitext(file.filename or "")[1]
        ...
        if "background_imagem" in request.files:
            ...
            if file and file.filename and allowed_file(file.filename):
                bg_filename = "background" + os.path.splitext(file.filename or "")[1]
...
def restaurar():
    ...
    filename = (file.filename or "").strip()
    if filename == "" or not filename.lower().endswith(".zip"):
        flash("Arquivo inválido. Por favor, envie um arquivo .zip.", "danger") # type: ignore
        return redirect(url_for("admin.configuracoes"))
...
def ignorar_sugestao(produto_id, sugestao_id):
    ...
    try:
        ...
        if request.method == "POST":
            if not existing:
                novo = SugestaoIgnorada()
                novo.produto_id = produto.id
                novo.sugestao_id = sugestao.id
                db.session.add(novo)
                db.session.commit()
