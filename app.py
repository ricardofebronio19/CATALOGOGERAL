...
def carregar_config_aparencia():
    ...
    try:
        ...
        return {
            ...
            'logo_path': None,
            'postgres_bin_path': '' # Novo campo para o caminho dos binários do PostgreSQL
        }
...
# Use a variável de ambiente DATABASE_URL se definida, senão use SQLite como padrão.
# Exemplo para PostgreSQL: DATABASE_URL=postgresql://user:password@host:port/dbname
DATABASE_URL = os.getenv('DATABASE_URL')
if DATABASE_URL and DATABASE_URL.startswith('postgres'):
    # Configurações para PostgreSQL
    from urllib.parse import quote_plus, urlparse, urlunparse
    
    # Analisa a URL para encontrar a senha e codificá-la corretamente.
    # Isso previne o UnicodeDecodeError se a senha tiver caracteres especiais.
    parsed_url = urlparse(DATABASE_URL)
    if parsed_url.password:
        safe_password = quote_plus(parsed_url.password)
        # Remonta a URL com a senha segura
        parsed_url = parsed_url._replace(netloc=f"{parsed_url.username}:{safe_password}@{parsed_url.hostname}:{parsed_url.port}")
    
    app.config['SQLALCHEMY_DATABASE_URI'] = urlunparse(parsed_url)
    app.config['SQLALCHEMY_ENGINE_OPTIONS'] = {'pool_pre_ping': True} # Mantém o pool_pre_ping
else:
...
def check_for_updates():
    ...
    try:
        # Usamos 'with app.app_context()' para garantir que o contexto da aplicação esteja disponível
        with app.app_context():
            ...
            latest_version_str = update_data.get("latest_version")
            
            # Usa a biblioteca packaging para uma comparação de versão robusta
            if latest_version_str and pkg_version.parse(latest_version_str) > pkg_version.parse(VERSION):
                print(f"Nova versão encontrada: {latest_version_str}")
                # Armazena as informações da atualização na configuração do app
...
class Aplicacao(db.Model):
    ...
    ano = db.Column(db.String(100), nullable=True)
...
def _apply_db_normalization(column):
    ...
    if is_postgres:
        # Usa a extensão unaccent do PostgreSQL para uma remoção de acentos eficiente.
        # A extensão precisa ser criada no banco: CREATE EXTENSION IF NOT EXISTS unaccent;
        normalized_column = func.unaccent(func.lower(column))
    ...
    # Remove pontuações comuns para ambos os bancos
    return func.replace(func.replace(func.replace(func.replace(normalized_column, '.', ''), '-', ''), ',', ''), ' ', '')

def _build_search_query(termo, codigo_produto, montadora, aplicacao_termo, grupo, medidas, sort_by='codigo', sort_order='asc'):
    """Constrói a query de busca de produtos com base nos filtros fornecidos."""
    query = Produto.query

    # Adiciona a ordenação ANTES de qualquer join ou distinct para garantir consistência.
    ...
    if termo:
        # A ordenação já foi aplicada, então agora podemos fazer o join e o distinct.
        # Nota: A ordenação foi movida para antes para evitar problemas com `distinct`.
    ...
    if codigo_produto:
        # Normaliza o termo de busca e a coluna do banco para uma comparação robusta
        codigo_normalizado = _normalize_for_search(codigo_produto)
        query = query.filter(_apply_db_normalization(Produto.codigo).contains(codigo_normalizado)) # Garante que a normalização seja usada
...
# --- Função customizada para SQLite ---
# Esta função será registrada no SQLite para permitir a verificação de sobreposição de anos no banco.
def _sqlite_ranges_overlap(year_str1, year_str2):
    """Função para ser usada dentro do SQLite. Converte strings de ano e verifica sobreposição."""
    range1 = _parse_year_range(year_str1)
    range2 = _parse_year_range(year_str2)
    # Retorna 1 para True, 0 para False, como o SQLite espera.
    return 1 if _ranges_overlap(range1, range2) else 0

# Registra a função customizada no SQLite quando a aplicação inicia.
# Isso só precisa ser feito uma vez.
@app.before_request
def register_sqlite_functions():
    if 'sqlite' in db.engine.url.drivername and not hasattr(app, '_sqlite_functions_registered'):
        db.engine.raw_connection().create_function("ranges_overlap", 2, _sqlite_ranges_overlap)
        app._sqlite_functions_registered = True
@app.route('/')
def index():
...
def buscar():
    ...
    sort_order = request.args.get('sort_order', 'asc') # Novo: Padrão para 'asc'

    # Define o número de itens por página
    PER_PAGE = 20

    # Usa a função auxiliar para construir a query
    # Passa os novos parâmetros de ordenação
    query = _build_search_query(termo, codigo_produto, montadora, aplicacao_termo, grupo, medidas, sort_by, sort_order)

    # A paginação é aplicada após a construção completa da query
...
def detalhe_peca(id):
    ...
    produto = db.session.query(Produto).options(
        ...
        # Usar joinedload para similares é mais eficiente para esta relação many-to-many.
        joinedload(Produto.similares).selectinload(Produto.aplicacoes), # Carrega aplicações dos similares
    ...
    if produto.grupo and produto.aplicacoes:
        ...
        # Constrói uma lista de filtros OR para a consulta.
        # Cada filtro corresponde a uma aplicação do produto principal.
        overlap_filters = []
        for app in produto.aplicacoes:
            if not app.veiculo: continue

            # Para cada aplicação do produto principal, cria um filtro que busca por
            # produtos candidatos que tenham uma aplicação para o mesmo veículo E
            # cujo intervalo de anos se sobreponha.
            # A função customizada 'ranges_overlap' é chamada aqui.
            overlap_filters.append(
                db.and_(
                    Aplicacao.veiculo == app.veiculo,
                    func.ranges_overlap(Aplicacao.ano, app.ano) == 1
                )
            )

        if overlap_filters:
            # Executa uma única consulta que já retorna os produtos correspondentes.
            sugestoes_similares = Produto.query.filter(
                Produto.id.notin_(ids_similares_manuais), # Exclui similares existentes
                Produto.grupo == produto.grupo, # Mesmo grupo
                Produto.aplicacoes.any(db.or_(*overlap_filters)) # Pelo menos uma aplicação correspondente
            ).options(selectinload(Produto.aplicacoes)).limit(10).all() # Limita a 10 sugestões
            sugestoes_similares_dict = {p.id: p for p in sugestoes_similares}
    ...
    return render_template('detalhe_peca.html', produto=produto, aplicacoes_agrupadas=aplicacoes_agrupadas, sugestoes_similares=list(sugestoes_similares_dict.values()), voltar_url=voltar_url)

def _get_postgres_executable_path(executable_name: str) -> str | None:
    """
    Encontra o caminho completo para um executável do PostgreSQL (ex: 'pg_dump').
    Usa shutil.which para procurar o executável no PATH do sistema.
    O PATH é modificado em run.py para incluir o caminho configurado pelo usuário.
    """
...
def configuracoes():
    ...
    if request.method == 'POST':
        ...
        # Novo: Salva o caminho para os binários do PostgreSQL
        config['postgres_bin_path'] = request.form.get('postgres_bin_path', '').strip()
...
def backup():
    ...
    if is_postgres:
        ...
        backup_filename = f"backup_catalogo_pg_{timestamp}.sql"
        
        pg_dump_executable = _get_postgres_executable_path('pg_dump')
        try:
            # Configura as variáveis de ambiente para a autenticação do pg_dump
            from urllib.parse import urlparse
            parsed_url = urlparse(app.config["SQLALCHEMY_DATABASE_URI"])
            
            env = os.environ.copy()
            env['PGHOST'] = parsed_url.hostname or 'localhost'
            env['PGPORT'] = str(parsed_url.port or 5432)
            env['PGUSER'] = parsed_url.username or ''
            env['PGPASSWORD'] = parsed_url.password or ''
            env['PGDATABASE'] = parsed_url.path.lstrip('/')

            # Constrói o comando de forma segura.
            # As variáveis de ambiente (PGHOST, PGPORT, PGUSER, PGPASSWORD, PGDATABASE)
            # são a forma padrão para o pg_dump se autenticar.
            # O comando final só precisa do executável e do nome do banco.
            dump_command = [pg_dump_executable, env['PGDATABASE']]

            # Verifica se o executável foi encontrado antes de tentar rodar
            if not pg_dump_executable:
                # Lança FileNotFoundError para ser pego pelo bloco except abaixo
                raise FileNotFoundError

            # Executa o comando. O check=True garante que um erro será lançado se o pg_dump falhar.
            process = subprocess.run(dump_command, capture_output=True, text=True, check=True, env=env, encoding='utf-8')
            ...
            flash("Erro: O executável 'pg_dump' não foi encontrado. Verifique se o caminho para os binários do PostgreSQL está configurado corretamente em 'Configurações' e se o PostgreSQL está instalado.", "danger")
            return redirect(url_for('configuracoes'))
        except subprocess.CalledProcessError as e:
            # A saída de erro do pg_dump é mais útil para o usuário.
            error_message = e.stderr or str(e)
            flash(f"Erro ao executar pg_dump: {error_message}. Verifique as configurações do banco de dados e se o PostgreSQL está instalado.", "danger")
            print(f"Erro detalhado do pg_dump: {e.stderr}")
            return redirect(url_for('configuracoes'))
...
def restaurar():
    ...
    is_postgres = 'postgresql' in db.engine.url.drivername
    if is_postgres:
        flash("A restauração para PostgreSQL deve ser feita manualmente pelo administrador do banco de dados usando 'psql' ou 'pg_restore'.", "info")
        return redirect(url_for('configuracoes'))

    # Lógica de restauração para SQLite (mantida)
...
def inicializar_banco():
    ...
    with app.app_context():
        ...
        with db.engine.connect() as connection:
            if is_sqlite:
                # Habilita o modo WAL (Write-Ahead Logging) para maior robustez do SQLite.
                connection.execute(db.text('PRAGMA journal_mode=WAL;'))
            elif is_postgres:
                # Garante que a extensão 'unaccent' exista no PostgreSQL para busca sem acentos.
                connection.execute(db.text('CREATE EXTENSION IF NOT EXISTS unaccent;'))
        db.create_all()
...
# --- Rota para Atualização da Aplicação ---
@app.route('/atualizar_aplicacao', methods=['POST'])
@login_required
def atualizar_aplicacao():
    ...
    update_info = app.config.get('UPDATE_INFO')
...
def _parse_e_salvar_aplicacoes_novo_formato(produto_alvo: Produto, nome_produto: str, texto_aplicacoes: str, grupo: str = None, fornecedor: str = None, conversoes: str = None, observacoes: str = None):
    """
    Interpreta uma string de aplicações (ex: "FIAT-PALIO 2010/2015 | GOL G5 2012/..."),
    extrai os veículos, anos e montadoras, e os salva no banco de dados.
    Também atualiza os dados principais do produto.
    """
    produto_alvo.nome = nome_produto
    # Atualiza campos adicionais se forem fornecidos
    if grupo: produto_alvo.grupo = grupo
    if fornecedor: produto_alvo.fornecedor = fornecedor
    # Apenas atualiza se um valor for passado, para não apagar dados existentes com uma célula vazia.
    if conversoes: produto_alvo.conversoes = conversoes.upper()
    if observacoes: produto_alvo.observacoes = observacoes.upper()

    # Limpa as aplicações antigas para evitar duplicatas ao reimportar
    if produto_alvo.id: produto_alvo.aplicacoes.clear()
    db.session.flush() # Garante que a exclusão seja processada antes de adicionar novos
    ...
    for aplicacao_str in texto_aplicacoes.split('|'):
        ...
        # 1. Encontra todos os padrões de ano na string
        # Padrões: 2010/2015, 2018/..., 2020
        year_pattern = r'(\b\d{4}(?:/\d{2,4}|/\.{3})?\b)'
        anos_encontrados = re.findall(year_pattern, aplicacao_str)

        # 2. Remove os anos da string para isolar o texto dos veículos
        veiculos_str = aplicacao_str
        for ano in anos_encontrados:
            veiculos_str = veiculos_str.replace(ano, '')
        veiculos_str = veiculos_str.strip()

        # 3. Extrai a montadora (se houver, no formato "MONTADORA-VEICULO")
        montadora = None
        if '-' in veiculos_str:
            partes = veiculos_str.split('-', 1)
            # Considera como montadora se a primeira parte for curta e toda em maiúsculas
            if len(partes[0]) < 15 and partes[0].isupper():
                montadora = partes[0].strip()
                veiculos_str = partes[1].strip()

        # 4. Divide os nomes dos veículos. A heurística é que nomes de veículos são separados por espaços,
        # mas nomes compostos (ex: "GOL G5") devem ser mantidos juntos.
        # A lógica aqui é simples: trata cada palavra como um veículo potencial,
        # mas a interface permite ao usuário corrigir depois.
        # Para uma lógica mais avançada, seria necessário um dicionário de veículos.
        # Por simplicidade, vamos assumir que o usuário pode separar com vírgula ou o script trata palavras.
        veiculos_potenciais = [v.strip() for v in re.split(r'[, ]+', veiculos_str) if v.strip()]

        # 5. Cria uma aplicação para cada veículo encontrado
        for veiculo_nome in veiculos_potenciais:
            # Remove a montadora do nome do veículo se já estiver lá
            if montadora and veiculo_nome.startswith(montadora):
                 veiculo_nome = veiculo_nome[len(montadora):].lstrip(' -')

            # Associa todos os anos encontrados na string a cada veículo
...
def importar_csv_route():
    ...
    try:
        ...
        # 1. Detectar o delimitador e preparar o leitor de CSV
        reader = None
        decoded_file = TextIOWrapper(file.stream, 'utf-8-sig', errors='ignore')

        # Tenta com delimitador ';'
        temp_reader = csv.DictReader(decoded_file, delimiter=';')
        if temp_reader.fieldnames and 'codigo' in [_normalize_for_search(h) for h in temp_reader.fieldnames]:
            reader = temp_reader
        else:
            # Se falhar, rebobina e tenta com ','
            file.stream.seek(0)
            # É crucial recriar o TextIOWrapper após o seek no stream original
            decoded_file = TextIOWrapper(file.stream, 'utf-8-sig', errors='ignore')
            reader = csv.DictReader(decoded_file, delimiter=',')

        # 2. Validar se o leitor foi criado e tem a coluna 'codigo'
        if not reader or not reader.fieldnames or 'codigo' not in [_normalize_for_search(h) for h in reader.fieldnames]:
            flash(f"ERRO: Não foi possível encontrar a coluna 'código' no arquivo '{file.filename}'. Verifique se o arquivo está formatado corretamente (CSV com delimitador ',' ou ';').", 'danger')
            return redirect(url_for('configuracoes'))

        # 3. Processar as linhas
        for row in reader:
            # Cria uma cópia do dicionário da linha com chaves normalizadas
            normalized_row = {
                _normalize_for_search(k.lstrip('\ufeff').strip('"')): v 
                for k, v in row.items() if k
            }

            get_value = lambda key: (normalized_row.get(key) or '').strip()
            ...
            fornecedor = (get_value('fornecedor') or get_value('fabricante') or get_value('marca')).strip().upper()
            
            # Novo: Busca por colunas de conversões e observações
            conversoes = (get_value('conversoes') or get_value('nconversoes') or get_value('conversao')).strip()
            observacoes = (get_value('observacoes') or get_value('obs')).strip()

            produto_existente = Produto.query.filter(func.upper(Produto.codigo) == codigo.upper()).first()
            ...
            _parse_e_salvar_aplicacoes_novo_formato(produto_alvo, nome_produto, texto_aplicacoes, grupo, fornecedor, conversoes, observacoes)
        ...
        print(f"Erro detalhado na importação: {e}") # Adiciona log no console para depuração
