...
def _get_form_datalists(app_context=None):
    ...
    def query_data():
        ...
        grupos_db = db.session.query(Produto.grupo).distinct().filter(Produto.grupo.isnot(None), Produto.grupo != '').order_by(Produto.grupo).all()
        lista_grupos = sorted([g[0] for g in grupos_db])

        fornecedores_db = db.session.query(Produto.fornecedor).distinct().filter(Produto.fornecedor.isnot(None), Produto.fornecedor != '').order_by(Produto.fornecedor).all()
        lista_fornecedores = sorted([f[0] for f in fornecedores_db])

        montadoras_db = db.session.query(Aplicacao.montadora).distinct().filter(Aplicacao.montadora.isnot(None), Aplicacao.montadora != '').all()
        ...
        return {"grupos": lista_grupos, "fornecedores": lista_fornecedores, "montadoras": lista_montadoras_completa}

    with _cache_lock:
        # Verifica se o cache existe e não expirou
        if _datalist_cache and (now - _datalist_cache.get("timestamp", 0)) < CACHE_TIMEOUT_SECONDS:
