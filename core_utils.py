...
def _is_code_like_term(text: str) -> bool:
    """Detecta se o termo parece um código de peça (ex: AL1084, AL-1084)."""
    if not text:
        return False

    import re

    # Tokens alfanuméricos separados por espaço.
    tokens = [t for t in re.split(r"\s+", text.strip()) if t]
    if not tokens:
        return False

    has_alpha_token = False
    has_digit_token = False

    for token in tokens:
        normalized = _normalize_code_for_search(token)
        if not normalized:
            continue
        has_letter = any(c.isalpha() for c in normalized)
        has_digit = any(c.isdigit() for c in normalized)
        has_alpha_token = has_alpha_token or (has_letter and not has_digit)
        has_digit_token = has_digit_token or (has_digit and not has_letter)
        if has_letter and has_digit:
            return True

    # Também trata código separado em dois tokens, ex: "AL 1084".
    return has_alpha_token and has_digit_token
...
def _normalize_code_for_search(text):
    ...
    nfkd_form = unicodedata.normalize("NFD", text.lower())
    text = "".join([c for c in nfkd_form if not unicodedata.combining(c)])
    # Remove tudo que não é letra/número para espelhar a normalização SQL.
    import re
...
def _build_search_query(
    ...
    if termo:
        # Força busca SQL para códigos (FTS5 pode falhar em códigos com/sem separadores).
        use_sql_search = "-" in termo or "/" in termo or _is_code_like_term(termo)
            ...
            for palavra in termo.strip().split():
                ...
                query = query.filter(
                    db.or_(
                        ...
                        _apply_code_normalization(Produto.codigo).contains(
                            palavra_codigo_normalizada
                        ),
    ...
    if codigo_produto:
        codigo_produto_normalizado = _normalize_code_for_search(codigo_produto)
        query = query.filter(
            db.or_(
                Produto.codigo.ilike(f"%{codigo_produto}%"),
                _apply_code_normalization(Produto.codigo).contains(
